...
// This release only supported combined SGW-U and PGW-U. they have same TAC and user plane id
// This release only support on one array item for selectors
// This release only support on one apn for the selector
// This release only support config: s1u, s5u_pgw,s5u_sgw
// This release not support entitlements
// This release, userplane id in the response body is valid. so the id from POST/userplane not useful.
// POST PGW
/*
URL:   http://192.168.120.219:10000/api/v1/pgwprofile?entity-type=pgw-dpf
Body:  Sample body for add
{
		"tac": "0x99",
		"apn_ni": "default.mnc092.mcc466.gprs",
		"s5u_ip": "192.168.100.100",
		"proxy_ip": "192.168.100.100"
} */

// POST SGW
/*
POST:  Add one dp entry to the existing pgw-dp-pool
=====

URL:   http://192.168.120.220:10000/api/v1/sgwprofile?entity-type=sgw-dpf
...
Body:  Sample body for add
{
		...
		"s5u_ip": "xx.xx.xx.xx",
		"s1u_ip": "xx.xx.xx.xx",
		"s1u_nat_ip": "xx.xx.xx.xx",
		"proxy_ip": "xx.xx.xx.xx"
}

s1u_nat_ip: we can ignore this field for now


example command:
curl -X POST -H "Content-Type: application/json" --data @sgw_dpf_add.json 
'http://192.168.120.220:10000/api/v1/sgwprofile?entity-type=sgw-dpf' | json_reformat


On successful operation, response body will have id value representing this ufp, 
this id value is equal to the value returned in pgw-cp POST API response. 
*/


// Q1: Sector ID not used
// Q2: TAC from MEC controller is int64. but from EPC, it is string with HEX format. for example: "tac" : "5a"
//	   so need to clarify with TAC that is HEX format
// Q3: UserPlane id is just the index of the UPF configuration array (some think like that.. )
//     the logic is written in both sgw and pgw same way.. to maintain the array indexes 
//     To SGW and PGW API, treat it as decimal value only.
// Q4: patch request: 204 updated user plane?? it should be not valid user plane ID.
/**
...
GET:   Get all the dpf info in the pgw-dp-pool
====
URL: http://192.168.120.219:10000/api/v1/pgwprofile?action=list&entity-type=pgw-dpf

example:
curl -X GET -H "Content-Type: application/json" 'http://192.168.120.219:10000/api/v1/pgwprofile?action=list&entity-type=pgw-dpf' | json_reformat
...
/*
GET:   Get the info for a specific dpf in the pgw-dp-pool using id
====
URL: http://192.168.120.219:10000/api/v1/pgwprofile?action=list&entity-type=pgw-dpf&id=xx

example:
curl -X GET -H "Content-Type: application/json" 'http://192.168.120.219:10000/api/v1/pgwprofile?action=list&entity-type=pgw-dpf&id=5' | json_reformat
...
/*
DELETE:   Delete one dp entry from the existing pool  (tac is used to identify the entry to delete)
=======

URL:  http://192.168.120.219:10000/api/v1/pgwprofile?entity-type=pgw-dpf&id=xx

example command:
curl -X DELETE -H "Content-Type: application/json" 'http://192.168.120.219:10000/api/v1/pgwprofile?entity-type=pgw-dpf&id=5' | json_reformat
*/
    /**
