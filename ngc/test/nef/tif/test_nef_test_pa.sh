...
if send_req post 0 json/AF_NEF_POST_01.json 201 AF_01; then
...
if send_req get $sub_id dummy_jsonn 200 AF_01; then
	echo "Passed" $sub_id # sub_id variable is set by the send_req func. 
else
	echo "Failed"
fi
...
if send_req patch $sub_id json/AF_NEF_PATCH_01.json 200 AF_01; then
	echo "Passed" $sub_id # sub_id variable is set by the send_req func. 
else
	echo "Failed"
fi
...
if send_req delete $sub_id dummy_jsonn 204 AF_01; then
	echo "Passed" $sub_id # sub_id variable is set by the send_req func. 
else
	echo "Failed"
fi
